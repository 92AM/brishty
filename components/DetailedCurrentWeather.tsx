      ...
      >
        <div className="lg:grid lg:grid-cols-5 lg:gap-9 divide-y divide-gray-500 lg:divide-y-0">
          <WeatherDescriptionItem
            description={'SUNRISE'}
            value={moment
              .unix(Number(weatherDetails.current.sunrise))
              .format('HH:MM')}
          />
          ...
          <WeatherDescriptionItem
            description={'SUNSET'}
            value={moment
              .unix(Number(weatherDetails.current.sunset))
              .format('HH:MM')}
          />
          ...
          <WeatherDescriptionItem
            ...
            value={`${Math.round(
              Number(weatherDetails.hourly[0].probabilityOfPrecipitation) * 100
            )}%`}
          />
          ...
          <WeatherDescriptionItem
            description={'WIND'}
            value={`${windDegreeToText(
              Number(weatherDetails.current.windDegree)
            )} ${Math.round(Number(weatherDetails.current.windSpeed))} km/hr`}
          />
        </div>
        ...
        <div className="lg:grid lg:grid-cols-5 lg:gap-9 divide-y divide-gray-500 lg:divide-y-0">
          <WeatherDescriptionItem
            ...
            value={`${weatherDetails.current.feelsLike}C`}
          />
          ...
          <WeatherDescriptionItem
            ...
            value={`${weatherDetails.current.pressure} hPa`}
          />
          ...
          <WeatherDescriptionItem
            ...
            value={weatherDetails.current.uvIndex}
          />
        </div>
      </div>
    </div>
